...
@section('title', 'Edit Registration Information')
...
<main class="flex-1 flex justify-center items-start px-4 py-10">
    <div class="w-full max-w-3xl bg-[#F2F6F9] rounded-lg shadow-xl p-6 text-slate-800">
        ...
        <h2 class="text-base font-semibold text-slate-900 mb-6">Edit Registration Information</h2>

        <form action="{{ route('admin.forauthor.update_registrations_admin', $registration->id) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="section" id="section"
                    value="{{ old('section', $registration->section ?? '') }}"
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="8"
                    class="w-full border border-gray-400 bg-gray-100 rounded-md px-3 py-2 focus:outline-none">{{ old('content', $registration->content ?? '') }}</textarea>
                @error('content')
